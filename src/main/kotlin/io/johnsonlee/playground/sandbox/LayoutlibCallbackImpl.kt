...
internal class LayoutlibCallbackImpl(
    ...
    private val appId: String by lazy {
        environment.appDir.resolve(SdkConstants.FN_ANDROID_MANIFEST_XML).takeIf {
            it.exists()
        }?.let { manifest ->
            AndroidManifestParser.parse(FileInputStream(manifest)).`package`
        } ?: environment.appDir.name
    }
    ...
    private fun initResources() {
        for (packageName in environment.resourcePackageNames) {
            val rClass = try {
